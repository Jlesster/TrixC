...
typedef struct {
  ...
  float opacity; /* 0 = unset, otherwise 0.0–1.0 */
} WinRule;
...
typedef struct {
  BarPos       position;
  int          height;
  int          padding;        /* horizontal padding on bar ends */
  int          glyph_y_offset; /* fine-tune text vertical position */
  ...
  bool         separator;
  bool         separator_top; /* draw separator on top edge instead of bottom */
...
typedef struct TrixieServer {
  ...
  struct wlr_session         *session; /* NULL when running nested/headless */
