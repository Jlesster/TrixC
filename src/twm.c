...
static Rect compute_rects(int sw, int sh, int bar_h, bool bar_bottom, int pad,
                           Rect *bar_out) {
	Rect content, bar;
	if (bar_h <= 0) {
...
void twm_set_bar_height(TwmState *t, int h, bool at_bottom) {
	t->content_rect = compute_rects(t->screen_w, t->screen_h, h, at_bottom,
...
void twm_reflow(TwmState *t) {
	...
	if (tiled_n > 0)
		layout_compute(ws->layout, t->content_rect, tiled_n,
		               ws->main_ratio, eff_gap, rects);
...
void twm_float_move(TwmState *t, PaneId id, int dx, int dy) {
	...
	if (p->rect.x + p->rect.w > t->screen_w)
		p->rect.x = t->screen_w - p->rect.w;
	if (p->rect.y + p->rect.h > t->screen_h)
		p->rect.y = t->screen_h - p->rect.h;
	p->float_rect = p->rect;
}
...
void twm_focus_dir(TwmState *t, int dx, int dy) {
	...
	for (int i = 0; i < ws->pane_count; i++) {
		...
		if (d2 < best_d2) { best_d2 = d2; best = pid; }
	}
	if (best) {
		ws->focused     = best;
		ws->has_focused = true;
...
void twm_swap(TwmState *t, bool forward) {
	...
	int cur = -1;
	for (int i = 0; i < ws->pane_count; i++)
		if (ws->panes[i] == ws->focused) { cur = i; break; }
	if (cur < 0) return;
