...
static void handle_kv(Config *c, ParseCtx *ctx, const char *key, const char *val) {
  ...
  if(ctx->depth == 0 || blk[0] == '\0') {
    if(!strcmp(key, "font"))
      ...
      /* parse mods+key */
      char combo_buf[128];
      strncpy(combo_buf, combo, sizeof(combo_buf) - 1);
      uint32_t mods     = 0;
      ...
      else if(!strcmp(action_str, "toggle_float"))
        act.kind = ACTION_TOGGLE_FLOAT;
      else if(!strcmp(action_str, "toggle_bar"))
      ...
      } else if(!strcmp(action_str, "move_to_workspace")) {
        act.kind = ACTION_MOVE_TO_WS;
        act.n    = arg1 ? atoi(arg1) : 1;
      } else if(!strcmp(action_str, "next_layout"))
        act.kind = ACTION_NEXT_LAYOUT;
      else if(!strcmp(action_str, "prev_layout"))
        act.kind = ACTION_PREV_LAYOUT;
      else if(!strcmp(action_str, "grow_main"))
        act.kind = ACTION_GROW_MAIN;
      else if(!strcmp(action_str, "shrink_main"))
        act.kind = ACTION_SHRINK_MAIN;
      else if(!strcmp(action_str, "next_workspace"))
        act.kind = ACTION_NEXT_WS;
      else if(!strcmp(action_str, "prev_workspace"))
        act.kind = ACTION_PREV_WS;
      else if(!strcmp(action_str, "quit"))
        act.kind = ACTION_QUIT;
      else if(!strcmp(action_str, "reload"))
  ...
  if(!strcmp(blk, "bar")) {
    ...
    if(!strcmp(key, "position"))
      b->position = !strcmp(val, "top") ? BAR_TOP : BAR_BOTTOM;
    else if(!strcmp(key, "height"))
      b->height = atoi(val);
    else if(!strcmp(key, "item_spacing"))
      b->item_spacing = atoi(val);
    else if(!strcmp(key, "pill_radius"))
      b->pill_radius = atoi(val);
    else if(!strcmp(key, "font_size"))
      b->font_size = atof(val);
    else if(!strcmp(key, "bg"))
      b->bg = parse_color(val);
    else if(!strcmp(key, "fg"))
      b->fg = parse_color(val);
    else if(!strcmp(key, "accent"))
      b->accent = parse_color(val);
    else if(!strcmp(key, "dim"))
      b->dim = parse_color(val);
    else if(!strcmp(key, "active_ws_fg"))
      b->active_ws_fg = parse_color(val);
    else if(!strcmp(key, "active_ws_bg"))
      b->active_ws_bg = parse_color(val);
    else if(!strcmp(key, "occupied_ws_fg"))
      b->occupied_ws_fg = parse_color(val);
    else if(!strcmp(key, "inactive_ws_fg"))
      b->inactive_ws_fg = parse_color(val);
    else if(!strcmp(key, "separator"))
      b->separator = !strcmp(val, "true");
    else if(!strcmp(key, "separator_color"))
      b->separator_color = parse_color(val);
      ...
      while(tok && b->modules_left_n < MAX_BAR_MODS)
        strncpy(b->modules_left[b->modules_left_n++], tok, 63),
            tok = strtok(NULL, " ,[]");
      ...
      while(tok && b->modules_center_n < MAX_BAR_MODS)
        strncpy(b->modules_center[b->modules_center_n++], tok, 63),
            tok = strtok(NULL, " ,[]");
      ...
      while(tok && b->modules_right_n < MAX_BAR_MODS)
        strncpy(b->modules_right[b->modules_right_n++], tok, 63),
            tok = strtok(NULL, " ,[]");
    }
    return;
  }
  ...
  if(!strcmp(blk, "colors")) {
    if(!strcmp(key, "active_border"))
      c->colors.active_border = parse_color(val);
    else if(!strcmp(key, "inactive_border"))
      c->colors.inactive_border = parse_color(val);
    else if(!strcmp(key, "pane_bg"))
      c->colors.pane_bg = parse_color(val);
    else if(!strcmp(key, "bar_bg"))
      c->bar.bg = parse_color(val);
    else if(!strcmp(key, "bar_fg"))
      c->bar.fg = parse_color(val);
    else if(!strcmp(key, "bar_accent"))
      c->bar.accent = parse_color(val);
    return;
  }
  ...
  if(!strcmp(blk, "keyboard")) {
    if(!strcmp(key, "layout"))
      ...
      c->keyboard.repeat_rate = atoi(val);
    else if(!strcmp(key, "repeat_delay"))
      c->keyboard.repeat_delay = atoi(val);
    return;
  }
    ...
    if(brace) {
      *brace = '\0';
      ...
      if(tok) {
        ...
        tok = strtok(NULL, " \t");
        if(tok)
          strncpy(label_stack[depth], tok, 63);
        else
          label_stack[depth][0] = '\0';
      }
      ...
      if(!strcmp(block_stack[depth], "scratchpad") &&
         c->scratchpad_count < MAX_SCRATCHPADS) {
