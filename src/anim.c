...
static Edge nearest_edge(Rect r, int sw, int sh) {
  ...
  if (fabsf(cx) >= fabsf(cy))
    return cx >= 0 ? EDGE_RIGHT : EDGE_LEFT;
...
static AnimEntry *find_entry(AnimSet *a, uint32_t id) {
  ...
  while (lo <= hi) {
    ...
    if (a->id_index[mid] == id)
      return &a->entries[a->idx_map[mid]];
    if (a->id_index[mid] < id)
      lo = mid + 1;
    else
      hi = mid - 1;
...
static void start_anim(AnimSet *a, uint32_t id, AnimKind kind,
                       const int from[4], Rect target, int duration_ms) {
...
void anim_fade_in(AnimSet *a, uint32_t id, int duration_ms) {
  ...
  if (!e)
    return;
  int from[4] = {e->anim.target.x, e->anim.target.y, e->anim.target.w,
                 e->anim.target.h};
...
float anim_get_opacity(AnimSet *a, uint32_t id, float fallback) {
  ...
  case ANIM_FLOAT_OPEN:
    return pa->opacity_from + range * ease_out_quint(t);
  case ANIM_FADE_OUT:
  case ANIM_FLOAT_CLOSE:
    return pa->opacity_from + range * ease_in_cubic(t);
  default:
    return fallback;
  }
}
...
void anim_morph(AnimSet *a, uint32_t id, Rect from, Rect to) {
  ...
  if (e && e->anim.active) {
    ...
    if (k == ANIM_OPEN || k == ANIM_CLOSE || k == ANIM_FLOAT_OPEN ||
        k == ANIM_FLOAT_CLOSE || k == ANIM_SCRATCH_OPEN ||
        k == ANIM_SCRATCH_CLOSE)
      return;
...
bool anim_tick(AnimSet *a) {
  ...
  if (a->ws.active) {
    float t =
        (float)((g_frame_ns - ((int64_t)a->ws.start.tv_sec * 1000000000LL +
                               a->ws.start.tv_nsec)) /
                1000000) /
...
Rect anim_get_rect(AnimSet *a, uint32_t id, Rect fallback) {
  ...
  case ANIM_SCRATCH_CLOSE:
    return lerp_rect(pa->from, tgt, ease_in_cubic(t));
...
bool anim_any(AnimSet *a) { return a->count > 0 || a->ws.active; }

bool anim_any_for_pane(AnimSet *a, PaneId id) {
  AnimEntry *e = find_entry(a, id);
  return e && e->anim.active;
}
...
int anim_ws_incoming_x(AnimSet *a) {
  if (!a->ws.active && a->ws.cached_e < 0.0f)
    return 0;
  if (!a->ws.active && a->ws.cached_e >= 1.0f)
    return 0;
