...
void anim_open(AnimSet *a, uint32_t id, Rect r) {
  ...
  int  off[4];
  edge_off(r, e, a->screen_w, a->screen_h, off);
...
/*
 * Two-pass tick: first mark finished anims inactive, then compact.
 * This ensures server_sync_windows sees every pane at its final rect
 * for at least one full frame before the entry is removed.
 */
bool anim_tick(AnimSet *a) {
  /* Pass 1: mark finished */
  for(int i = 0; i < a->count; i++) {
    PaneAnim *pa = &a->entries[i].anim;
    if(!pa->active) continue;
    if(anim_progress(pa) >= 1.0f) pa->active = false;
  }

  /* Pass 2: compact out inactive entries */
  int live = 0;
  for(int i = 0; i < a->count; i++) {
...
Rect anim_get_rect(AnimSet *a, uint32_t id, Rect fallback) {
  ...
  switch(pa->kind) {
    case ANIM_OPEN:
    case ANIM_SCRATCH_OPEN: return lerp_rect(pa->from, tgt, ease_out_quint(t));

    case ANIM_CLOSE:
    case ANIM_SCRATCH_CLOSE: return lerp_rect(tgt, pa->from, ease_in_cubic(t));
