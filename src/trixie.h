...
typedef struct {
	...
	int duration_ms;
	Rect target;        /* on-screen destination */
	int from[4];        /* off-screen / prev: x,y,w,h */
	bool active;
} PaneAnim;
...
typedef enum {
	...
	ACTION_TOGGLE_FLOAT,
	ACTION_TOGGLE_BAR,
	ACTION_FOCUS_LEFT, ACTION_FOCUS_RIGHT, ACTION_FOCUS_UP, ACTION_FOCUS_DOWN,
...
typedef struct {
	...
	int  exec_argc;
	int  n;           /* workspace / vt number */
	char name[64];    /* scratchpad name */
} Action;
...
typedef struct {
	...
	Rect     rect;
	Rect     float_rect;
	/* wlroots surface (opaque pointer, cast in compositor code) */
	void    *surface;   /* struct wlr_xdg_toplevel * */
} Pane;
...
/* Lifecycle */
void twm_init(TwmState *t, int w, int h, int bar_h, bool bar_bottom,
              int gap, int border_w, int pad, int ws_count, bool smart_gaps);
...
/* Layout */
void   layout_compute(Layout l, Rect area, int n, float ratio, int gap,
                      Rect *out);

// ── Bar renderer (Cairo-based, drawn into wlr_scene) ─────────────────────────

typedef struct TrixieBar TrixieBar;
TrixieBar *bar_create(struct wlr_scene *scene, int w, int h,
                      const Config *cfg);
...
typedef struct TrixieServer {
	...
	struct wl_list views;        /* TrixieView::link  */
	struct wl_list outputs;      /* TrixieOutput::link */
	struct wl_list keyboards;    /* TrixieKeyboard::link */
	...
	/* IPC socket fd registered in event loop */
	int        ipc_fd;
...
TrixieView *view_at(TrixieServer *s, double lx, double ly,
