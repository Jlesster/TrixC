...
static void handle_foreign_toplevel_request_activate(struct wl_listener *listener, void *data) {
  (void)data;
...
static void handle_foreign_toplevel_request_close(struct wl_listener *listener, void *data) {
  (void)data;
...
void server_set_bar_inset(TrixieServer *s, int h, bool at_bottom) {
  if (at_bottom) s->layer_excl_bottom = h;
  else           s->layer_excl_top    = h;
...
void server_sync_windows(TrixieServer *s) {
  ...
  wl_list_for_each(v, &s->views, link) {
    ...
    /* Fix 2: un-suspend now that the window is visible. */
    if (!v->is_xwayland && v->xdg_toplevel)
      ...
      if (v->xdg_toplevel) {
        if (!anim_any_for_pane(&s->anim, v->pane_id) &&
          ...
          if (cur.width != cw || cur.height != ch) {
            /* Fix 1: configure_resent guard — prevents infinite configure
             * loops with terminals that round to cell boundaries and Electron
             * apps that ignore our size hint. */
            if (v->configure_resent) {
              /* Client already rendered at a different size after our last
               * configure — accept the mismatch, stop looping. */
              v->configure_resent = false;
            } else {
              /* First mismatch: send the correct size and arm the guard so
               * we only re-send once before accepting whatever the client
               * actually renders. */
              v->configure_resent =
...
void server_dispatch_action(TrixieServer *s, Action *a) {
...
#ifdef HAS_XWAYLAND
        if (v->is_xwayland && v->xwayland_surface) {
          wlr_xwayland_surface_close(v->xwayland_surface); break;
        }
#endif
  ...
  case ACTION_FULLSCREEN: {
    ...
    if (p) {
      ...
      TrixieView *v = view_from_pane(s, id);
      if (v) {
        struct wlr_scene_tree *tgt = p->fullscreen ? s->layer_floating : s->layer_windows;
  ...
  case ACTION_TOGGLE_BAR:
    ...
    {
      TrixieOutput *o;
      wl_list_for_each(o, &s->outputs, link) bar_set_visible(o->bar, s->twm.bar_visible);
    }
    server_reflow_with_morph(s);
    break;
  ...
  case ACTION_MOVE_LEFT:   TWM_MORPH_SWAP(s, twm_swap_dir(&s->twm, -1,  0)); break;
  case ACTION_MOVE_RIGHT:  TWM_MORPH_SWAP(s, twm_swap_dir(&s->twm,  1,  0)); break;
  case ACTION_MOVE_UP:     TWM_MORPH_SWAP(s, twm_swap_dir(&s->twm,  0, -1)); break;
  case ACTION_MOVE_DOWN:   TWM_MORPH_SWAP(s, twm_swap_dir(&s->twm,  0,  1)); break;
  case ACTION_SWAP_MAIN:   TWM_MORPH_SWAP(s, twm_swap_main(&s->twm)); break;
...
void server_float_toggle(TrixieServer *s) {
  ...
  if (p->floating && !was_float) anim_float_open(&s->anim, id, p->rect);
  else if (!p->floating && was_float) anim_open(&s->anim, id, p->rect);
  server_sync_windows(s);
  server_sync_focus(s);
  server_mark_deco_dirty(s);
}
...
void server_scratch_toggle(TrixieServer *s, const char *name) {
  ...
  for (int i = 0; i < s->twm.scratch_count; i++)
    if (!strcmp(s->twm.scratchpads[i].name, name)) { sp = &s->twm.scratchpads[i]; break; }
  ...
  if (p) {
    if (!was_visible && sp->visible) anim_scratch_open(&s->anim, pid, p->rect);
    else if (was_visible && !sp->visible) anim_scratch_close(&s->anim, pid, p->rect);
  }
  server_sync_windows(s);
  server_sync_focus(s);
  server_mark_deco_dirty(s);
...
static int reload_pipe_cb(int fd, uint32_t mask, void *data) {
  ...
  if (mask & WL_EVENT_READABLE) {
    ...
    if (n > 0) { buf[n] = '\0'; wlr_log(WLR_INFO, "hot-reload: %s", buf); }
  }
  if (mask & (WL_EVENT_HANGUP | WL_EVENT_ERROR)) {
    int status = 0;
    pid_t done = waitpid(s->reload_pid, &status, WNOHANG);
    if (s->reload_pipe_src) { wl_event_source_remove(s->reload_pipe_src); s->reload_pipe_src = NULL; }
    close(fd);
    s->reload_pipe_fd = -1;
    s->reload_pid = -1;
    int exit_code = (done > 0 && WIFEXITED(status)) ? WEXITSTATUS(status) : 1;
    if (exit_code != 0) { wlr_log(WLR_ERROR, "hot-reload: ninja failed (%d)", exit_code); return 0; }
    ...
    wlr_backend_destroy(s->backend); s->backend = NULL;
...
void server_binary_reload(TrixieServer *s) {
  if (s->reload_pid > 0) return;
  ssize_t len = readlink("/proc/self/exe", s->reload_new_bin, sizeof(s->reload_new_bin) - 1);
  if (len <= 0) return;
  ...
  if (s->cfg.build_dir[0]) {
    ...
    if (readlink("/proc/self/exe", self, sizeof(self) - 1) > 0) {
      ...
      if (slash) { *slash = '\0'; strncpy(build_dir, self, sizeof(build_dir) - 1); }
    }
...
static void deferred_reload_cb(void *data) {
  TrixieServer *s = data;
  if (s->reload_idle) { wl_event_source_remove(s->reload_idle); s->reload_idle = NULL; }
  server_apply_config_reload(s);
}
void server_schedule_reload(TrixieServer *s) {
  if (s->reload_idle) return;
...
static uint32_t wlr_mods_to_trixie(uint32_t m) {
  uint32_t out = 0;
  if (m & WLR_MODIFIER_LOGO)  out |= MOD_SUPER;
  if (m & WLR_MODIFIER_CTRL)  out |= MOD_CTRL;
  if (m & WLR_MODIFIER_ALT)   out |= MOD_ALT;
  if (m & WLR_MODIFIER_SHIFT) out |= MOD_SHIFT;
  return out;
}
...
static void keyboard_handle_key(struct wl_listener *listener, void *data) {
  ...
  if (s->locked) {
    ...
    wlr_seat_keyboard_notify_key(s->seat, event->time_msec, event->keycode, event->state);
    return;
  }
#endif
  ...
  int nbase = xkb_keymap_key_get_syms_by_level(kb->wlr_keyboard->keymap, keycode, 0, 0, &base_syms);
  ...
  if (event->state == WL_KEYBOARD_KEY_STATE_PRESSED) {
    for (int i = 0; i < nsyms; i++)
      if (syms[i] == XKB_KEY_Print && (mods & MOD_SUPER) && (mods & MOD_SHIFT)) {
        s->running = false; wl_display_terminate(s->display); return;
      }
...
static void server_new_keyboard(TrixieServer *s, struct wlr_input_device *dev) {
  ...
  kb->server = s; kb->wlr_keyboard = wlr_kb;
  ...
  xkb_keymap_unref(keymap); xkb_context_unref(ctx);
  wlr_keyboard_set_repeat_info(wlr_kb, s->cfg.keyboard.repeat_rate, s->cfg.keyboard.repeat_delay);
...
static void server_new_pointer(TrixieServer *s, struct wlr_input_device *dev) {
  ...
  if (!tptr) return;
  tptr->server = s; tptr->wlr_pointer = ptr;
  tptr->swipe_begin.notify  = handle_swipe_begin;  wl_signal_add(&ptr->events.swipe_begin,  &tptr->swipe_begin);
  tptr->swipe_update.notify = handle_swipe_update; wl_signal_add(&ptr->events.swipe_update, &tptr->swipe_update);
  tptr->swipe_end.notify    = handle_swipe_end;    wl_signal_add(&ptr->events.swipe_end,    &tptr->swipe_end);
  tptr->pinch_begin.notify  = handle_pinch_begin;  wl_signal_add(&ptr->events.pinch_begin,  &tptr->pinch_begin);
  tptr->pinch_update.notify = handle_pinch_update; wl_signal_add(&ptr->events.pinch_update, &tptr->pinch_update);
  tptr->pinch_end.notify    = handle_pinch_end;    wl_signal_add(&ptr->events.pinch_end,    &tptr->pinch_end);
  tptr->destroy.notify      = pointer_handle_destroy; wl_signal_add(&dev->events.destroy,   &tptr->destroy);
...
static void handle_new_input(struct wl_listener *listener, void *data) {
  ...
  case WLR_INPUT_DEVICE_TABLET_PAD:
    wlr_cursor_attach_input_device(s->cursor, dev); break;
  default: break;
  }
...
static bool super_is_held(TrixieServer *s) {
  TrixieKeyboard *kb;
  wl_list_for_each(kb, &s->keyboards, link)
    if (wlr_keyboard_get_modifiers(kb->wlr_keyboard) & WLR_MODIFIER_LOGO) return true;
  return false;
}
...
static void update_cursor_focus(TrixieServer *s, uint32_t time_msec) {
  if (s->drag_mode != DRAG_NONE) return;
...
static void handle_cursor_motion(struct wl_listener *listener, void *data) {
  ...
  if (s->drag_mode == DRAG_MOVE) {
    ...
    if (p) { twm_float_move(&s->twm, s->drag_pane, (int)ev->delta_x, (int)ev->delta_y); server_sync_windows(s); server_request_redraw(s); return; }
  }
  if (s->drag_mode == DRAG_RESIZE) {
    Pane *p = twm_pane_by_id(&s->twm, s->drag_pane);
    if (p) { twm_float_resize(&s->twm, s->drag_pane, (int)ev->delta_x, (int)ev->delta_y); server_sync_windows(s); server_request_redraw(s); return; }
  }
...
static void handle_cursor_button(struct wl_listener *listener, void *data) {
  ...
  uint32_t mods = 0;
  TrixieKeyboard *kb;
  wl_list_for_each(kb, &s->keyboards, link) mods |= wlr_keyboard_get_modifiers(kb->wlr_keyboard);
  uint32_t lmods = 0;
  if (mods & WLR_MODIFIER_LOGO)  lmods |= MOD_SUPER;
  if (mods & WLR_MODIFIER_CTRL)  lmods |= MOD_CTRL;
  if (mods & WLR_MODIFIER_ALT)   lmods |= MOD_ALT;
  if (mods & WLR_MODIFIER_SHIFT) lmods |= MOD_SHIFT;
  uint32_t btn = ev->button == BTN_LEFT ? 1 : ev->button == BTN_MIDDLE ? 2 : ev->button == BTN_RIGHT ? 3 : ev->button;
  if (lua_dispatch_button(s, lmods, btn, pressed)) return;

  wlr_seat_pointer_notify_button(s->seat, ev->time_msec, ev->button, ev->state);
  if (!pressed) { s->drag_mode = DRAG_NONE; s->drag_pane = 0; return; }
  ...
  if (pid) {
    ...
    if (held && ev->button == BTN_LEFT && is_float) { s->drag_mode = DRAG_MOVE; s->drag_pane = pid; anim_cancel(&s->anim, pid); return; }
    if (held && ev->button == BTN_RIGHT && is_float) { s->drag_mode = DRAG_RESIZE; s->drag_pane = pid; anim_cancel(&s->anim, pid); return; }
  }
}
...
static void handle_request_set_primary_selection(struct wl_listener *listener, void *data) {
  TrixieServer *s = CONTAINER_OF(listener, TrixieServer, seat_request_set_primary_selection);
...
typedef struct { struct wl_listener destroy; } DragIconCtx;
...
static void handle_start_drag(struct wl_listener *listener, void *data) {
  ...
  if (!drag->icon) return;
  struct wlr_scene_tree *t = wlr_scene_drag_icon_create(s->layer_overlay, drag->icon);
  if (!t) return;
  wlr_scene_node_set_position(&t->node, (int)s->cursor->x, (int)s->cursor->y);
  g_drag_icon_tree = t;
  DragIconCtx *ctx = calloc(1, sizeof(*ctx));
  if (ctx) { ctx->destroy.notify = drag_icon_handle_destroy; wl_signal_add(&drag->icon->events.destroy, &ctx->destroy); }
}
...
static void output_handle_frame(struct wl_listener *listener, void *data) {
  ...
  for (int wi = 0; wi < o->wibox_count; wi++) {
    ...
    if (!wb || !wb->visible || !wb->scene_buf || !wb->dirty) continue;
    if (wb->lua_draw_ref != LUA_NOREF) wibox_lua_draw(wb, s->L);
    else wibox_commit(wb);
    bar_dirty = true;
  }
  ...
  if (fid) {
    ...
    if (fp && fp->fullscreen) {
      TrixieView *fv;
      wl_list_for_each(fv, &s->views, link) {
        if (fv->pane_id != fid || !fv->mapped) continue;
        struct wlr_surface *surf = NULL;
#ifdef HAS_XWAYLAND
        if (fv->is_xwayland && fv->xwayland_surface) surf = fv->xwayland_surface->surface;
        else
#endif
        if (fv->xdg_toplevel) surf = fv->xdg_toplevel->base->surface;
        if (surf) {
          enum wp_tearing_control_v1_presentation_hint hint =
              wlr_tearing_control_manager_v1_surface_hint_from_surface(s->tearing_mgr, surf);
          if (hint == WP_TEARING_CONTROL_V1_PRESENTATION_HINT_ASYNC) want_tearing = true;
        }
        break;
      }
    }
  ...
  /* Fix 3: if a tiled pane has a pending configure AND its process is stopped
   * (SIGSTOP/SIGTSTP), skip the scene commit — the stopped process will never
   * ack so committing just wastes GPU time. */
  {
    TrixieView *fv;
    bool skip = false;
    wl_list_for_each(fv, &s->views, link) {
      if (!fv->mapped || !fv->xdg_toplevel || fv->is_xwayland) continue;
      Pane *fp = twm_pane_by_id(&s->twm, fv->pane_id);
      if (!fp || fp->floating || fp->fullscreen) continue;
      if (!fv->configure_resent) continue;
      pid_t cpid = 0;
      wl_client_get_credentials(
          fv->xdg_toplevel->base->client->client, &cpid, NULL, NULL);
      if (cpid > 0) {
        siginfo_t si = {0};
        if (waitid(P_PID, (id_t)cpid, &si,
                   WNOHANG | WCONTINUED | WSTOPPED | WNOWAIT) == 0 &&
            si.si_pid == cpid &&
            (si.si_code == CLD_STOPPED || si.si_code == CLD_TRAPPED)) {
          skip = true;
          break;
        }
      }
    ...
    if (skip)
      goto frame_done_only;
  }

  if (use_shader) {
    shader_render_frame(&o->shader, s->renderer, o->scene_output, o->wlr_output, sat);
  } else {
    struct wlr_output_state state;
    wlr_output_state_init(&state);
    if (wlr_scene_output_build_state(o->scene_output, &state, NULL)) {
      if (want_tearing) state.tearing_page_flip = true;
...
frame_done_only:
  /* Fix 3: stopped-process path — skip commit but still send frame_done so
   * wp_presentation feedback reaches other clients. */
  ;
...
static void output_handle_request_state(struct wl_listener *listener, void *data) {
  ...
  if (o->width != nw || o->height != nh) {
    o->width = nw; o->height = nh; o->logical_w = nw; o->logical_h = nh;
    if (o->bar) bar_resize(o->bar, nw, nh);
    if (s->bg_rect) wlr_scene_rect_set_size(s->bg_rect, nw, nh);
    bool is_primary = false;
    if (!wl_list_empty(&s->outputs)) {
      TrixieOutput *first = wl_container_of(s->outputs.next, first, link);
      is_primary = (first == o);
    }
    if (is_primary) { twm_resize(&s->twm, nw, nh); anim_set_resize(&s->anim, nw, nh); server_sync_windows(s); }
    server_mark_deco_dirty(s);
  }
}
...
static void output_handle_destroy(struct wl_listener *listener, void *data) {
  ...
  for (int wi = 0; wi < o->wibox_count; wi++) wibox_destroy(o->wiboxes[wi]);
  o->wibox_count = 0;
...
static void handle_new_output(struct wl_listener *listener, void *data) {
  ...
  if (!wlr_output_test_state(wlr_output, &state)) {
    wlr_log(WLR_DEBUG, "output %s: adaptive sync not supported", wlr_output->name);
  ...
  o->server = s; o->wlr_output = wlr_output;
  o->frame.notify = output_handle_frame;         wl_signal_add(&wlr_output->events.frame,         &o->frame);
  o->request_state.notify = output_handle_request_state; wl_signal_add(&wlr_output->events.request_state, &o->request_state);
  o->destroy.notify = output_handle_destroy;     wl_signal_add(&wlr_output->events.destroy,       &o->destroy);
  wl_list_insert(&s->outputs, &o->link);

  struct wlr_output_layout_output *lo = wlr_output_layout_add_auto(s->output_layout, wlr_output);
  ...
  o->width = ow; o->height = oh; o->logical_w = ow; o->logical_h = oh;

  TrixieOutput *primary = wl_container_of(s->outputs.next, primary, link);
  if (primary == o) { twm_resize(&s->twm, ow, oh); anim_set_resize(&s->anim, ow, oh); }
...
static void layer_surface_handle_map(struct wl_listener *listener, void *data) {
  ...
  bool at_top    = (anchor & ZWLR_LAYER_SURFACE_V1_ANCHOR_TOP)    && !(anchor & ZWLR_LAYER_SURFACE_V1_ANCHOR_BOTTOM);
  bool at_bottom = (anchor & ZWLR_LAYER_SURFACE_V1_ANCHOR_BOTTOM) && !(anchor & ZWLR_LAYER_SURFACE_V1_ANCHOR_TOP);
  if (at_top)    s->layer_excl_top    = exclusive;
  if (at_bottom) s->layer_excl_bottom = exclusive;
  if (at_top || at_bottom) server_set_bar_inset(s, exclusive, at_bottom);
}
static void layer_surface_handle_unmap(struct wl_listener *listener, void *data) {
  ...
  uint32_t anchor = wls->current.anchor;
  bool at_bottom = (anchor & ZWLR_LAYER_SURFACE_V1_ANCHOR_BOTTOM) && !(anchor & ZWLR_LAYER_SURFACE_V1_ANCHOR_TOP);
  if (at_bottom) s->layer_excl_bottom = 0; else s->layer_excl_top = 0;
  if (s->layer_excl_bottom > 0) server_set_bar_inset(s, s->layer_excl_bottom, true);
  else if (s->layer_excl_top > 0) server_set_bar_inset(s, s->layer_excl_top, false);
  else server_set_bar_inset(s, 0, false);
}
static void layer_surface_handle_destroy(struct wl_listener *listener, void *data) {
  ...
  wl_list_remove(&ls->map.link); wl_list_remove(&ls->unmap.link);
  wl_list_remove(&ls->destroy.link); wl_list_remove(&ls->link);
  free(ls);
}
static void handle_new_layer_surface(struct wl_listener *listener, void *data) {
  ...
  case ZWLR_LAYER_SHELL_V1_LAYER_BACKGROUND: layer_tree = s->layer_background; break;
  case ZWLR_LAYER_SHELL_V1_LAYER_BOTTOM:     layer_tree = s->layer_windows;    break;
  case ZWLR_LAYER_SHELL_V1_LAYER_TOP:        layer_tree = s->layer_top;        break;
  default:                                   layer_tree = s->layer_overlay;    break;
  }
  TrixieLayerSurface *ls = calloc(1, sizeof(*ls));
  ls->server = s; ls->wlr_surface = wls;
  ...
  if (!out && !wl_list_empty(&s->outputs)) { TrixieOutput *o = wl_container_of(s->outputs.next, o, link); out = o->wlr_output; }
  if (out) {
    struct wlr_box ob;
    wlr_output_layout_get_box(s->output_layout, out, &ob);
    wlr_layer_surface_v1_configure(wls, (uint32_t)ob.width, (uint32_t)ob.height);
  }
...
static void view_do_map(TrixieServer *s, TrixieView *v, Pane *p,
  ...
  for (int i = 0; i < s->twm.scratch_count; i++)
    if (s->twm.scratchpads[i].has_pane && s->twm.scratchpads[i].pane_id == v->pane_id) { is_scratch = true; break; }
...
static void view_handle_unmap(struct wl_listener *listener, void *data) {
  ...
  for (int i = 1; i <= 16; i++) {
    ...
    if (nv && nv->mapped) { twm_set_focused(&s->twm, prev); server_focus_pane(s, prev); return; }
  }
  TrixieView *next;
  wl_list_for_each(next, &s->views, link)
    if (next != v && next->mapped) { server_focus_pane(s, next->pane_id); break; }
}
...
static void view_handle_commit(struct wl_listener *listener, void *data) {
  ...
  wl_list_for_each(o, &s->outputs, link) {
    ...
    Pane *p = twm_pane_by_id(&s->twm, v->pane_id);
    if (p) {
      int cx = p->rect.x + p->rect.w / 2, cy = p->rect.y + p->rect.h / 2;
      struct wlr_output *at = wlr_output_layout_output_at(s->output_layout, cx, cy);
      if (at == o->wlr_output) wlr_fractional_scale_v1_notify_scale(surf, o->wlr_output->scale);
    } else {
  ...
  if (p) {
    ...
    wl_list_for_each(o, &s->outputs, link) {
      struct wlr_output *at = wlr_output_layout_output_at(s->output_layout, cx, cy);
      if (at == o->wlr_output) wlr_surface_send_enter(surf, o->wlr_output);
      else wlr_surface_send_leave(surf, o->wlr_output);
    }
  }
}
...
static void view_handle_set_title(struct wl_listener *listener, void *data) {
  ...
  if (!v->xdg_toplevel || !v->xdg_toplevel->title) return;
  const char *new_title = v->xdg_toplevel->title;
  twm_set_title(&v->server->twm, v->pane_id, new_title);
  if (v->foreign_handle) wlr_foreign_toplevel_handle_v1_set_title(v->foreign_handle, new_title);
...
static void view_handle_set_app_id(struct wl_listener *listener, void *data) {
  ...
  if (!v->xdg_toplevel || !v->xdg_toplevel->app_id) return;
  Pane *p = twm_pane_by_id(&v->server->twm, v->pane_id);
  if (p) strncpy(p->app_id, v->xdg_toplevel->app_id, sizeof(p->app_id) - 1);
  twm_try_assign_scratch(&v->server->twm, v->pane_id, v->xdg_toplevel->app_id);
  if (v->foreign_handle) wlr_foreign_toplevel_handle_v1_set_app_id(v->foreign_handle, v->xdg_toplevel->app_id);
}
...
static void handle_new_xdg_surface(struct wl_listener *listener, void *data) {
  ...
  v->pane_id = pane_id;
  v->configure_resent = false; /* Fix 1: initialise guard */
#ifdef HAS_XWAYLAND
  v->is_xwayland = false;
...
static void xwayland_handle_set_title(struct wl_listener *listener, void *data) {
  ...
  if (v->foreign_handle) wlr_foreign_toplevel_handle_v1_set_title(v->foreign_handle, t);
...
static void xwayland_handle_set_class(struct wl_listener *listener, void *data) {
  ...
  if (v->foreign_handle) wlr_foreign_toplevel_handle_v1_set_app_id(v->foreign_handle, c);
}
...
static void handle_xwayland_ready(struct wl_listener *listener, void *data) {
  ...
  if (s->cfg.cursor_theme[0]) setenv("XCURSOR_THEME", s->cfg.cursor_theme, true);
  char sz[8]; snprintf(sz, sizeof(sz), "%d", s->cfg.cursor_size > 0 ? s->cfg.cursor_size : 24);
...
static void lock_surface_handle_map(struct wl_listener *l, void *data) {
  ...
  if (!ls->scene_tree) ls->scene_tree = wlr_scene_tree_create(ls->server->layer_lock);
  struct wlr_surface *surf = wlr_session_lock_surface_v1_get_surface(ls->wlr_surface);
  if (surf && ls->scene_tree) wlr_scene_surface_create(ls->scene_tree, surf);
  if (ls->scene_tree) wlr_scene_node_set_enabled(&ls->scene_tree->node, true);
  struct wlr_keyboard *kb = wlr_seat_get_keyboard(ls->server->seat);
  if (surf && kb) wlr_seat_keyboard_notify_enter(ls->server->seat, surf, kb->keycodes, kb->num_keycodes, &kb->modifiers);
}
static void lock_surface_handle_destroy(struct wl_listener *l, void *data) {
  (void)data;
  TrixieLockSurface *ls = wl_container_of(l, ls, destroy);
  wl_list_remove(&ls->map.link); wl_list_remove(&ls->destroy.link);
  if (ls->scene_tree) wlr_scene_node_destroy(&ls->scene_tree->node);
  wl_list_remove(&ls->link); free(ls);
}
typedef struct {
  TrixieServer *server;
...
static void lock_handle_new_surface(struct wl_listener *l, void *data) {
  ...
  ls->server = lk->server; ls->wlr_surface = wls; ls->scene_tree = NULL;
  TrixieServer *s = lk->server;
  if (!wl_list_empty(&s->outputs)) {
    TrixieOutput *o = wl_container_of(s->outputs.next, o, link);
    wlr_session_lock_surface_v1_configure(wls, (uint32_t)o->width, (uint32_t)o->height);
  }
  ls->map.notify = lock_surface_handle_map;     wl_signal_add(&wls->events.map, &ls->map);
  ls->destroy.notify = lock_surface_handle_destroy; wl_signal_add(&wls->events.destroy, &ls->destroy);
...
static void lock_handle_unlock(struct wl_listener *l, void *data) {
  ...
  wl_list_for_each(ls, &lk->surfaces, link) wlr_scene_node_set_enabled(&ls->scene_tree->node, false);
  PaneId id = twm_focused_id(&s->twm);
  if (id) server_focus_pane(s, id);
}
static void lock_handle_destroy(struct wl_listener *l, void *data) {
  ...
  lk->server->session_lock = NULL; lk->server->locked = false;
  wl_list_remove(&lk->new_surface.link); wl_list_remove(&lk->unlock.link); wl_list_remove(&lk->destroy.link);
  free(lk); g_lock = NULL;
}
static void handle_session_lock_new_lock(struct wl_listener *l, void *data) {
  ...
  if (s->locked) { wlr_session_lock_v1_destroy(lock); return; }
  TrixieLock *lk = calloc(1, sizeof(*lk));
  lk->server = s; lk->wlr_lock = lock; wl_list_init(&lk->surfaces);
  lk->new_surface.notify = lock_handle_new_surface; wl_signal_add(&lock->events.new_surface, &lk->new_surface);
  lk->unlock.notify = lock_handle_unlock;           wl_signal_add(&lock->events.unlock,      &lk->unlock);
  lk->destroy.notify = lock_handle_destroy;         wl_signal_add(&lock->events.destroy,     &lk->destroy);
  s->session_lock = lock; s->locked = true; g_lock = lk;
  wlr_session_lock_v1_send_locked(lock);
}
static void handle_session_lock_manager_destroy(struct wl_listener *l, void *data) {
  (void)data;
  TrixieServer *s = wl_container_of(l, s, session_lock_lock_destroy);
  s->locked = false; s->session_lock = NULL;
}
#endif
...
static void handle_output_mgr_apply(struct wl_listener *l, void *data) {
  ...
  wl_list_for_each(head, &cfg->heads, link) {
    ...
    if (!wlr_output_commit_state(head->state.output, &state)) ok = false;
    wlr_output_state_finish(&state);
  }
...
static void handle_output_mgr_test(struct wl_listener *l, void *data) {
  ...
  wl_list_for_each(head, &cfg->heads, link) {
    struct wlr_output_state state; wlr_output_state_init(&state);
    wlr_output_head_v1_state_apply(&head->state, &state);
    if (!wlr_output_test_state(head->state.output, &state)) ok = false;
    wlr_output_state_finish(&state);
  }
  if (ok) wlr_output_configuration_v1_send_succeeded(cfg);
  else    wlr_output_configuration_v1_send_failed(cfg);
...
static int ipc_read_cb(int fd, uint32_t mask, void *data) {
  ...
  char buf[65536] = {0}; size_t total = 0; ssize_t n;
  ...
  if (total > 0) {
    ...
    if (!strncmp(cmd, "subscribe", 9) && (cmd[9] == '\0' || cmd[9] == ' ' || cmd[9] == '\t')) {
      if (ipc_subscribe(s, client)) return 0;
      write_all_fd(client, "err: subscriber table full\n", 27); close(client); return 0;
    }
    if (!strncmp(cmd, "binary_reload", 13) && (cmd[13] == '\0' || cmd[13] == '\n')) {
      server_binary_reload(s); write_all_fd(client, "ok: building\n", 13); close(client); return 0;
    }
    char reply[65536] = {0};
...
void server_init_ipc(TrixieServer *s) {
  ...
  listen(fd, 8); fcntl(fd, F_SETFL, O_NONBLOCK);
  s->ipc_fd = fd;
...
static int inotify_cb(int fd, uint32_t mask, void *data) {
  ...
  while (p < end) {
    ...
    if (ev->len > 0) { size_t nl = strlen(ev->name); if (nl >= 4 && strcmp(ev->name + nl - 4, ".lua") == 0) { relevant = true; break; } }
    p += sizeof(struct inotify_event) + ev->len;
  }
  if (!relevant) return 0;
  struct timespec now; clock_gettime(CLOCK_MONOTONIC, &now);
  int64_t now_ms = now.tv_sec * 1000 + now.tv_nsec / 1000000;
  if (now_ms - last_reload_ms < 500) return 0;
...
void server_init_config_watch(TrixieServer *s) {
  ...
  else { const char *home = getenv("HOME"); if (!home) home = "/root"; snprintf(dir, sizeof(dir), "%s/.config/trixie", home); }
  s->inotify_fd = inotify_init1(IN_NONBLOCK);
  if (s->inotify_fd < 0) return;
  if (inotify_add_watch(s->inotify_fd, dir, IN_MODIFY | IN_CREATE | IN_MOVED_TO) < 0) { close(s->inotify_fd); s->inotify_fd = -1; return; }
  s->inotify_src = wl_event_loop_add_fd(wl_display_get_event_loop(s->display), s->inotify_fd, WL_EVENT_READABLE, inotify_cb, s);
...
static void crash_handler(int sig) {
  ...
  char sigbuf[32]; int sl = snprintf(sigbuf, sizeof(sigbuf), "signal: %d\n", sig);
...
int main(int argc, char *argv[]) {
  ...
  wl_list_init(&s->keyboards); wl_list_init(&s->layer_surfaces);
  wl_list_init(&s->pointers);
  s->running = true; s->ipc_fd = -1; s->inotify_fd = -1;
  s->reload_pid = -1; s->reload_pipe_fd = -1; s->saved_argv = argv;
  ...
  wl_signal_add(&s->cursor_shape_mgr->events.request_set_shape, &s->cursor_shape_request);

  s->xdg_activation = wlr_xdg_activation_v1_create(s->display);
  s->xdg_activation_request.notify = handle_xdg_activation;
  wl_signal_add(&s->xdg_activation->events.request_activate, &s->xdg_activation_request);
  ...
  wl_signal_add(&s->pointer_constraints->events.new_constraint, &s->new_pointer_constraint);
  ...
  if (s->idle_inhibit_mgr) {
    ...
    wl_signal_add(&s->idle_inhibit_mgr->events.new_inhibitor, &s->idle_inhibit_new);
  }
  ...
  {
    struct wlr_xdg_foreign_registry *foreign_reg = wlr_xdg_foreign_registry_create(s->display);
    wlr_xdg_foreign_v1_create(s->display, foreign_reg);
    wlr_xdg_foreign_v2_create(s->display, foreign_reg);
  }
#endif
#ifdef HAVE_DRM_LEASE_V1
  if (wlr_backend_is_drm(s->backend)) wlr_drm_lease_v1_manager_create(s->display, s->backend);
#endif
#if __has_include(<wlr/types/wlr_session_lock_manager_v1.h>)
  ...
  wl_signal_add(&s->session_lock_mgr->events.new_lock, &s->session_lock_new_lock);
  s->session_lock_lock_destroy.notify = handle_session_lock_manager_destroy;
  wl_signal_add(&s->session_lock_mgr->events.destroy, &s->session_lock_lock_destroy);
#endif
  ...
  wl_signal_add(&s->seat->events.request_set_primary_selection, &s->seat_request_set_primary_selection);
  ...
  if (!wlr_backend_start(s->backend)) { wlr_log(WLR_ERROR, "failed to start backend"); return 1; }
  g_server = s;
  ...
  if (s->cfg.idle_timeout > 0) {
    s->idle_timeout_ms = s->cfg.idle_timeout * 1000;
    s->idle_timer = wl_event_loop_add_timer(wl_display_get_event_loop(s->display), idle_timer_cb, s);
    wl_event_source_timer_update(s->idle_timer, s->idle_timeout_ms);
  }
  ...
  if (s->reload_idle)   wl_event_source_remove(s->reload_idle);
