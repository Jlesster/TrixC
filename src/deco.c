...
static bool tbuf_begin(struct wlr_buffer *b,
                       uint32_t           flags,
                       void             **data,
                       uint32_t          *fmt,
                       size_t            *stride) {
...
static void ensure_rects(DecoEntry *e, struct wlr_scene_tree *layer) {
  if(e->has_rects && e->layer == layer) return;
  destroy_rects(e);
  if(e->label) {
...
static void hide_entry(DecoEntry *e) {
  ...
  if(e->label) wlr_scene_node_set_enabled(&e->label->node, false);
...
/* ── rect positioning ───────────────────────────────────────────────────── */
static void position_rects(DecoEntry *e, Rect r, int bw, float tc[4], float sc[4]) {
  ...
  if(gs && cs && e->last_enabled) return;
  e->last_rect = r;
...
/* ── label upload ───────────────────────────────────────────────────────── */
static void update_label(DecoEntry             *e,
                         struct wlr_scene_tree *layer,
                         Rect                   r,
                         int                    bw,
                         int                    ws_idx,
  ...
  if(fh <= 0) return; /* font not ready */

  /* Build label string */
  char        text[320];
  const char *name = (p->app_id[0]) ? p->app_id : (p->title[0]) ? p->title : "?";
  ...
  if(!changed && !moved && e->last_enabled && e->label) return;
  ...
  if(!e->label) e->label = wlr_scene_buffer_create(layer, NULL);

  if(changed) {
    Color pb = cfg->colors.background;
    Color fg = focused ? cfg->colors.active_border : cfg->colors.inactive_border;
  ...
  /* Center vertically on the top border line — always reposition to follow anims */
  e->last_label_x = lx;
  e->last_label_y = ly;
...
void deco_destroy(TrixieDeco *d) {
  ...
  for(int i = 0; i < d->count; i++) {
    ...
    if(d->entries[i].label) wlr_scene_node_destroy(&d->entries[i].label->node);
  }
  free(d);
}
...
void deco_update(TrixieDeco *d, TwmState *twm, AnimSet *anim, const Config *cfg) {
  ...
  for(int i = 0; i < ws->pane_count; i++) {
    ...
    bool noborder = false;
    if(bw > 0) {
      for(int ri = 0; ri < cfg->win_rule_count && !noborder; ri++) {
        const WinRule *wr = &cfg->win_rules[ri];
        if(!wr->app_id[0] || !wr->noborder) continue;
        bool match = (!strncmp(wr->app_id, "title:", 6))
                         ? strstr(p->title, wr->app_id + 6) != NULL
                         : strstr(p->app_id, wr->app_id) != NULL;
        if(match) noborder = true;
      }
    }
    if(bw <= 0 || noborder) {
      DecoEntry *e = deco_find(d, pid);
    ...
    struct wlr_scene_tree *layer   = p->floating ? d->layer_float : d->layer_tiled;
    ...
    update_label(
        e, layer, shifted, fbw, twm->active_ws, focused, p->floating, cfg, p);
  }
  for(int i = 0; i < d->count; i++)
