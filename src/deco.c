/* deco.c — Window border decorations using wlr_scene colored rects. */
#include "trixie.h"
#include <stdlib.h>
#include <string.h>
...
typedef struct {
  PaneId                 id;
  bool                   active;
  struct wlr_scene_rect *borders[4]; /* 0=top 1=bottom 2=left 3=right */
  bool                   has_rects;
} DecoEntry;
...
static void ensure_borders(TrixieDeco *d, DecoEntry *e) {
  ...
  for(int k = 0; k < 4; k++) {
    float col[4]  = { 0, 0, 0, 1 };
    e->borders[k] = wlr_scene_rect_create(d->layer, 1, 1, col);
    wlr_scene_node_set_enabled(&e->borders[k]->node, false);
  }
  e->has_rects = true;
}

static void position_borders(
    DecoEntry *e, Rect r, int bw, bool focused, Color active, Color inactive) {
...
void deco_update(TrixieDeco *d, TwmState *twm, AnimSet *anim, const Config *cfg) {
  ...
  for(int i = 0; i < ws->pane_count; i++) {
    ...
    ensure_borders(d, e);
    position_borders(
        e, r, bw, focused, cfg->colors.active_border, cfg->colors.inactive_border);
  }

  for(int i = 0; i < d->count; i++)
    if(!d->entries[i].active) hide_borders(&d->entries[i]);
}
