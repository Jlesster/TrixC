...
static void draw_char(uint32_t *px, int stride_px, int x, int y,
                      char c, uint32_t color, int clip_h) {
	...
	for (int col = 0; col < 5; col++) {
		for (int row = 0; row < FONT_H; row++) {
			...
			if ((glyph[col] >> row) & 1)
				px[(y + row) * stride_px + x + col] = color;
		}
	}
}

static void draw_text(uint32_t *px, int stride_px, int x, int y,
                      const char *s, uint32_t color, int clip_w, int clip_h) {
	while (*s) {
		if (x >= clip_w) break;
...
static void fill_rect_px(uint32_t *px, int stride_px,
                          int x, int y, int w, int h,
                          uint32_t color, int clip_w, int clip_h) {
...
TrixieBar *bar_create(struct wlr_scene *scene, int w, int h, const Config *cfg) {
	...
	b->image  = pixman_image_create_bits(PIXMAN_a8r8g8b8, bw, bh,
	                                     b->pixels, bw * 4);
...
static int draw_bar_item(uint32_t *px, int stride, const BarItem *it,
                          int x, int y, int bar_h, Color bar_bg,
                          int pill_r) {
	...
	draw_text(px, stride, x + it->pad, ty, it->text,
	          to_argb(it->fg), stride, bar_h);
...
static void read_net_label(char *out, int max) {
	...
	if (!f) { strncpy(out, "eth0", max); return; }
	char iface[32], line[256];
	fgets(line, sizeof(line), f); /* header */
	while (fgets(line, sizeof(line), f)) {
		unsigned flags;
		if (sscanf(line, "%31s %*s %*s %x", iface, &flags) == 2) {
			if (flags & 0x2) { strncpy(out, iface, max); fclose(f); return; }
		}
	}
	fclose(f);
...
#define MAX_ITEMS 32
	...
	/* Layout */
	{
		...
		if (right_n < MAX_ITEMS)
			right_items[right_n++] = (BarItem){
				.text = layout_buf, .fg = bar->accent, .pad = 2,
			};
	}
	...
	/* Battery */
	{
		...
		if (pct >= 0) {
			...
			Color fg = (pct <= 20 && !charging)
			    ? (Color){0xf3,0x8b,0xa8,0xff} : bar->fg;
			if (right_n < MAX_ITEMS)
	...
	/* Network */
	{
		char iface[32]; read_net_label(iface, sizeof(iface));
		char state_path[128];
		snprintf(state_path, sizeof(state_path),
		         "/sys/class/net/%s/operstate", iface);
		FILE *f = fopen(state_path, "r");
		if (f) {
			char state[16]; fscanf(f, "%15s", state); fclose(f);
	...
	for (int i = 0; i < left_n; i++)
		x += draw_bar_item(px, stride, &left_items[i],
		                   x, 0, bh, bar->bg, bar->pill_radius);

	/* Right zone (right-aligned) */
	int right_w = bar->item_spacing > 0 ? bar->item_spacing : 4;
	for (int i = 0; i < right_n; i++)
		right_w += bar_item_width(&right_items[i]) + (bar->item_spacing > 0 ? bar->item_spacing : 4);
	int rx = bw - right_w;
	for (int i = 0; i < right_n; i++) {
		int sp = bar->item_spacing > 0 ? bar->item_spacing : 4;
		rx += draw_bar_item(px, stride, &right_items[i],
		                    rx, 0, bh, bar->bg, bar->pill_radius);
		rx += sp;
	}
	...
	for (int i = 0; i < center_n; i++)
		cx += draw_bar_item(px, stride, &center_items[i],
		                    cx, 0, bh, bar->bg, bar->pill_radius);
